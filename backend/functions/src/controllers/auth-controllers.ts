...
export const syncFirebaseUser = async (req: Request, res: Response, next: NextFunction) => {
    const { firebase_uid, email, username, profile_pic } = req.body;
    ...
    try {
        ...
        if (userCheck.rows.length > 0) {
            // Update username or profile_pic if needed
            const updateQuery = `
                UPDATE users SET username = $1, profile_pic = $2, updated_at = NOW()
                WHERE firebase_uid = $3
                RETURNING *;
            `;
            const updatedUser = await pool.query(updateQuery, [username, profile_pic ?? null, firebase_uid]);
        ...
        `;
        const insertResult = await pool.query(insertQuery, [firebase_uid, email, username, profile_pic ?? null]);
